      ...
      <div class="container-fluid">
        ...
        <div class="row">
              ...
              <div class="card-header border-0">
                <h3 class="card-title"><?php echo get_text_by_lang('Email List','admin');?></h3>
...
function prepareShowData()
{
    ...
    for (var i = 0; i < total; i++) {
        li+='<tr class="tr-id-'+pageData['theList'][i]['email']+'||'+pageData['theList'][i]['group_id']+'">';
        li+='<td><button type="button" class="btn btn-default btn-xs btn-checkbox" data-checked="no" data-id="'+pageData['theList'][i]['email']+'||'+pageData['theList'][i]['group_id']+'"><i class="fas fa-square"></i></button></td>';
        li+='<td>'+pageData['theList'][i]['email']+'</td>';
...
$(document).on('click','.btnSearch',function(){
  ...
  sendData['type']='1';
  pageData['group_id']=$('.search-group > option:selected').val().trim();
  pageData['keywords']=$('.search-keywords').val().trim();
  sendData['group_id']=pageData['group_id'];
  sendData['keywords']=pageData['keywords'];

  postData(API_URL+'get_list_email_marketing_emailist', sendData).then(data => {
    // console.log(data); // JSON data parsed by `data.json()` call
    $('#modalSearch').modal('hide');
