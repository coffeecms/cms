      ...
      <div class="container-fluid">
        ...
        <div class="row">
              ...
              <div class="card-header border-0">
                <h3 class="card-title"><?php echo get_text_by_lang('Pages','admin');?></h3>
...
function prepareShowPost()
{
  ...
  for (var i = 0; i < total; i++) {
    ...
    li+='<tr class="tr-id-'+pageData['listPost'][i]['page_c']+'">';
    li+='<td><button type="button" class="btn btn-default btn-xs btn-checkbox" data-checked="no" data-id="'+pageData['listPost'][i]['page_c']+'"><i class="fas fa-square"></i></button></td>';
    li+='<td><a style="color:#000;" href="'+SITE_URL+'admin/edit_page?page_c='+pageData['listPost'][i]['page_c']+'" target="_blank" title="'+pageData['listPost'][i]['title']+'">'+pageData['listPost'][i]['title']+'</td>';
...
$(document).on('click','.btnApply',function(){
  ...
  postData(API_URL+'page_action_apply', sendData).then(data => {
    // console.log(data); // JSON data parsed by `data.json()` call
...
$(document).on('click','.btnSearch',function(){
  ...
  pageData['page_type']=$('.search-post-type > option:selected').val().trim();
  ...
  sendData['page_type']=pageData['page_type'];
  sendData['title']=pageData['title'];
  sendData['username']=pageData['username'];

  postData(API_URL+'get_list_page', sendData).then(data => {
    // console.log(data); // JSON data parsed by `data.json()` call
    $('#modalSearch').modal('hide');
