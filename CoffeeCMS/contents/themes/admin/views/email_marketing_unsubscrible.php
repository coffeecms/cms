      ...
      <div class="container-fluid">
        ...
        <div class="row">
              ...
              <div class="card-header border-0">
                <h3 class="card-title"><?php echo get_text_by_lang('Unsubscrible list','admin');?></h3>
...
function prepareShowData()
{
    ...
    for (var i = 0; i < total; i++) {
        ...
        li+='<tr class="tr-id-'+pageData['theList'][i]['id']+'">';
        li+='<td><button type="button" class="btn btn-default btn-xs btn-checkbox" data-checked="no" data-id="'+pageData['theList'][i]['id']+'"><i class="fas fa-square"></i></button></td>';
...
$(document).on('click','.btnApply',function(){
  ...
  postData(API_URL+'email_marketing_unsubscrible_action_apply', sendData).then(data => {
    // console.log(data); // JSON data parsed by `data.json()` call
...
$(document).on('click','.btnSearch',function(){
  ...
  sendData['type']='1';
  sendData['keywords']=$('.search-keywords').val().trim();

  postData(API_URL+'get_list_email_marketing_unsubscrible', sendData).then(data => {
    // console.log(data); // JSON data parsed by `data.json()` call
    $('#modalSearch').modal('hide');
